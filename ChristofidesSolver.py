...
#                graphe.add_edge(j,i, weight = matriceCost[j-1][i])
	...
	def graphe_impairs(self,tsp, mst):
		"""
	...
	def solve(self,tsp,chQueue,step):
		...
		*******************************
		...
		MImpairs = self.graphe_impairs(tsp, MST)
		chQueue.put(("self.updateChristofides",MImpairs))

		if step:
			self.block()
		chQueue.put(("self.updateChristofides",tsp.getVertices()))
		...
		if step:
			self.block()
		chQueue.put(("self.updateChristofides","hi"))
